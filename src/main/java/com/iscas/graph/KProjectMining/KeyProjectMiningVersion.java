...
public class KeyProjectMiningVersion {
    ...
    private HashMap<String, Double> unitImportanceDegree; // 单位重要度

    public KeyProjectMiningVersion() {
        ...
        // 单位重要度
        unitImportanceDegree = new HashMap<String, Double>();
    }

    public KeyProjectMiningVersion(Double ROIWeight,
    ...
    public void run(SubsystemData subsystemData) {
        ...
        for (Map.Entry<String, Double> entity: investmentvolumeanalysis.ROI.entrySet()) {
            // 项目名称
            String projectName = entity.getKey();
            // 投资回报率
            Double ROI = entity.getValue();
            // 实际效益
            Double actualBenefit = investmentvolumeanalysis.actualBenefits.get(projectName);
            // 能力列表
            List<String> abilities = investmentvolumeanalysis.project2ability.get(projectName);
            // 能力个数
            Double abilityNum = (double)abilities.size();
            // 能力支撑度
            Double supportDegree = investmentvolumeanalysis.capacitySupportDegree.get(projectName);
            // 项目重要度
            Double projectImpDeg = ROI * ROIWeight + actualBenefit * actualBenefitWeight + abilityNum * supportCapabilityNumWeight + supportDegree * capabilitySupportDegreeWeight;
            projectImportanceDegree.put(projectName, projectImpDeg);
        }

        for (Map.Entry<String, Double> res: projectImportanceDegree.entrySet()) {
            System.out.println("Key:" + res.getKey() + "||" + "Value:" + res.getValue());
        }
        //// 三.骨干单位挖掘
        // 结合项目重要度，进行分数相加，分数越高，单位越重要
        // 骨干单位挖掘
        for (Map.Entry<String, List<String>> entity: indicatorsAnalysis.unitProject.entrySet()) {
            System.out.println("Key:" + entity.getKey() + "||" + "Value:" + entity.getValue());
            Double unitImpSCore = 0.0;
            List<String> projects = entity.getValue();
            for (int i = 0; i < projects.size(); i++) {
                unitImpSCore += projectImportanceDegree.get(projects.get(i));
            }
            unitImportanceDegree.put(entity.getKey(), unitImpSCore);
        }
//        for (Map.Entry<String, Double> entity: unitImportanceDegree.entrySet()) {
//            System.out.println("Key:" + entity.getKey() + "||" + "Value:" + entity.getValue());
//        }
    ...
    public List<Map<String, Object>> dataFormatting(KeyProjectMiningVersion keyProjectMiningVersion){ // 投资回报率
        /**
         * 返回结果：
         * {
         *     项目名称: 1
        ...
        projectData.put("keyProject", keyProjectMiningVersion.projectImportanceDegree);
        projectData.put("keyUnit", keyProjectMiningVersion.unitImportanceDegree);
    ...
    public static void main(String[] args) {
        ...
        keyProjectMiningVersion.run(InputData);
